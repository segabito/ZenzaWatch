...
<div class="scrollToTop command" title="一番上にスクロール" data-command="scrollToTop">&#x2303;</div>
  ...
  */});
  ...
  CommentListItemView.__css__ = ZenzaWatch.util.hereDoc(function() {/*
    * {
    ...
    .videoItem.noCommentCounter .counter {
      display: none;
    }
    .videoItem .counter {
      font-size: 12px;
  ...
  */});
  ...
  CommentListItem.createByThumbInfo = function(info) {
  ...
  CommentListItem.createBlankInfo = function(id) {
  ...
  CommentListItem.createByMylistItem = function(item) {
    if (item.item_data) {
  ...
  CommentListItem.createByCommentInfoModel = function(info) {
  ...
  var RelatedCommentList = function() { this.initialize.apply(this, arguments); };
  _.extend(RelatedCommentList.prototype, CommentList.prototype);
  _.assign(RelatedCommentList.prototype, {
    update: function(listData, watchId) {
      //window.console.log('RelatedCommentList: ', listData, watchId);
      if (!this._view) { this._initializeView(); }
      this._watchId = watchId;
      var items = [];
      _.each(listData, function(itemData) {
  ...
  _.extend(PlaylistModel.prototype, CommentListModel.prototype);
  ...
  */});
  ...
  _.extend(Playlist.prototype, CommentList.prototype);
  _.assign(Playlist.prototype, {
      ...
      return this._mylistApiLoader
        .getMylistItems(mylistId, options).then(function(items) {
          ...
          _.each(items, function(item) {
            ...
            videoListItems.push(
              CommentListItem.createByMylistItem(item)
            );
          });
    ...
    insert: function(watchId) {
      ...
      return this._thumbInfoLoader.load(watchId).then(function (info) {
        ...
        info.id = watchId;
        var item = CommentListItem.createByThumbInfo(info);
      ...
      function(result) {
        var item = CommentListItem.createBlankInfo(watchId);
        model.insertItem(item, index + 1);
        this._refreshIndex(true);
    ...
    insertCurrentComment: function(videoInfo) {
      this._initializeView();

      //window.console.log('insertCurrentComment', videoInfo);
      if (this._activeItem &&
          !this._activeItem.isBlankData() &&
          this._activeItem.getWatchId() === videoInfo.getWatchId()) {
        this._activeItem.setIsPlayed(true);
        this.scrollToActiveItem();
        //window.console.log('insertCurrentComment.getWatchId() === videoInfo.getWatchId()');
        return;
      }
      ...
      var item = CommentListItem.createByCommentInfoModel(videoInfo);
      item.setIsPlayed(true);
